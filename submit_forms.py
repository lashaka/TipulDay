...
class submit_forms(object):
    ...
    def main(self: object) -> None:
        """
        This function is used to submit forms to the websites from the websites list.
        """
        ...
        if len(self.websites) > 0:
            for website in self.websites:
                ...
                for data in form_action:
                    ...
                    if (
                        action == f"{Domain}{const.HTTPS_HTTP_SLASH_TEXT[2]}"
                        or action
                        == f"{const.HTTPS_HTTP_SLASH_TEXT[0]}{Domain}{const.HTTPS_HTTP_SLASH_TEXT[2]}"
                        or action
                        == f"{const.HTTPS_HTTP_SLASH_TEXT[1]}{Domain}{const.HTTPS_HTTP_SLASH_TEXT[2]}"
                    ):
                        continue
                    else:
                        try:
                            ...
                            continue
                form_elements = soup.find_all(const.HERF_AND_A_TEXT[4]) + soup.find_all(
                    const.HERF_AND_A_TEXT[5]
                )
                form_data = {}
                ...
                for data in form_data:
                    try:
                        if (
                            const.FORM_DATA_TEXT[0] in data
                            or const.FORM_DATA_TEXT[4] in data
                            or const.FORM_DATA_TEXT[8] in data
                            or const.FORM_DATA_TEXT[12] in data
                        ):
                            form_data[data] = self.FORM_NAME
                        if (
                            const.FORM_DATA_TEXT[1] in data
                            or const.FORM_DATA_TEXT[5] in data
                            or const.FORM_DATA_TEXT[9] in data
                            or const.FORM_DATA_TEXT[13] in data
                            or const.FORM_DATA_TEXT[16] in data
                            or const.FORM_DATA_TEXT[18] in data
                        ):
                            form_data[data] = self.FORM_PHONE
                        if (
                            const.FORM_DATA_TEXT[2] in data
                            or const.FORM_DATA_TEXT[6] in data
                            or const.FORM_DATA_TEXT[10] in data
                            or const.FORM_DATA_TEXT[14] in data
                        ):
                            form_data[data] = self.FORM_EMAIL
                        if (
                            const.FORM_DATA_TEXT[3] in data
                            or const.FORM_DATA_TEXT[7] in data
                            or const.FORM_DATA_TEXT[11] in data
                            or const.FORM_DATA_TEXT[15] in data
                            or const.FORM_DATA_TEXT[17] in data
                            or const.FORM_DATA_TEXT[19] in data
                        ):
